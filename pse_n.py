...
def train(config):
    ...
    for step in range(steps):
        ...
        fin = open(data_config["test"]["list_path"])
        fout = open(config["pse_file"], "w")
        for i, (t_imgs, t_labels) in enumerate(dset_loaders["test"]):
        ...
        t_pse_set = ImageList(open(config["pse_file"]).readlines(),
                              transform=prep_dict["target"])
        dset_loaders_pse = DataLoader(t_pse_set, batch_size=config["data"]["source1"]["batch_size"],
                                      shuffle=True, num_workers=4)
        log_str = "pse len: {:05d}".format(len(t_pse_set))
