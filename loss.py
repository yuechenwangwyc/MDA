...
def GVB(softmax_out, ad_net, coeff=None, myloss=Myloss()):
    ...
    return myloss(ad_out,dc_target,weight.view(-1, 1))
...
def GVB2(softmax_out, ad_net, coeff=None, myloss=Myloss2()):
    softmax_output = softmax_out
    ad_out = ad_net(softmax_output)
    ad_out = nn.Sigmoid()(ad_out)
    batch_size = softmax_output.size(0) // 2
    ...
    return myloss(ad_out,dc_target)
